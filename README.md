...
python pix2pix.py \
  ...
  --output_dir facades_train \
  --max_epochs 100 \
  --input_dir my/combined/train \
  --which_direction AtoB
  
python pix2pixNew.py \
  --mode test \
  --output_dir facades_test \
  --input_dir my/combined/val \
  --checkpoint facades_train
...
# Resize source images
python tools/process.py \
  --input_dir my/original \
  --operation resize \
  --output_dir my/resized
  
# Create images with blank centers
python tools/process.py \
  --input_dir my/resized \
  --operation blank \
  --output_dir my/blank
